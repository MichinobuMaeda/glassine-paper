 ...
 * />
 */
export const NavDrawer = ({ id, items = [], className, }) => {
    return (_jsx("div", { id: id, className: `nav-drawer ${className || ''}`, children: Array.from(items).map((item, index) => (_jsx(NavDrawerItem, { ...item }, item.key || index))) }));
};
