 ...
 *
 */
export const Button = ({ id, name, variant = 'filled', size = 'md', label, icon, width = 'default', radius = 'default', type = 'button', href, checked = false, disabled = false, onClick, onChange, className = '', style = {}, }) => {
    const classes = [
        'button',
        label ? '' : 'icon',
        variant,
        ...
        .filter(Boolean)
        .join(' ');
    return type === 'toggle' ? (_jsxs("label", { id: `${id}-label`, className: classes, style: style, children: [_jsx("input", { id: id, name: name, type: "checkbox", checked: checked, disabled: disabled, onChange: onChange }), icon, label] })) : type === 'select' ? (_jsxs("label", { id: `${id}-label`, className: classes, style: style, children: [_jsx("input", { id: id, name: name, type: "radio", checked: checked, disabled: disabled, onChange: onChange }), icon, label] })) : href ? (_jsxs("a", { id: id, href: href, className: classes, style: style, children: [icon, label] })) : (_jsxs("button", { id: id, name: name, type: type, className: classes, disabled: disabled, onClick: onClick, style: style, children: [icon, label] }));
};
