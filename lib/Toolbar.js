...
export const Toolbar = ({ id, items = [], style, variant = 'docked', color = 'standard', }) => {
    ...
    return variant === 'floating' ? (_jsxs("div", { className: "toolbar-container", style: style, children: [_jsx("div", { id: id, className: classes, children: activeItems
                    ...
                    .map((item, index) => (_jsx(ToolbarItem, { ...item }, item.key || index))) }), activeItems
                .filter((item) => item.fab)
                .map((item, index) => (_jsx(ToolbarItem, { ...item }, item.key || index)))] })) : (_jsx("div", { id: id, className: classes, style: style, children: activeItems.map((item, index) => (_jsx(ToolbarItem, { ...item, fab: false }, item.key || index))) }));
};
