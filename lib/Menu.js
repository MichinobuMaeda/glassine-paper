...
const MenuItem = ({ id, label, leadingIcon, trailingIcon, divider = false, active = false, disabled = false, onClick, href, }) => {
...
export const Menu = ({ id, items = [], onClose = () => { }, className, style, }) => {
    return (_jsx("div", { id: id, className: `menu ${className || ''}`, onClick: onClose, style: style, children: Array.from(items)
            .filter((item) => !!item)
            .map((item, index) => {
            const { key: itemKey, ...rest } = item;
            const key = itemKey ?? index;
            return _jsx(MenuItem, { ...rest }, key);
        }) }));
};
export default Menu;
